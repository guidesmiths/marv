...
var debug = require('debug')('marv:index')
...
module.exports = function migrate(migrations, driver, cb) {
    ...
    function getDeltas(existingMigrations, cb) {
        ...
        cb(null, _.chain(migrations).filter(function(migration) {
            ...
            var check = watermark ? checksum(watermark.checksum + migration.script) : checksum(migration.script)
            return _.merge({ timestamp: new Date(), checksum: check }, migration)
        }).value())
    }
