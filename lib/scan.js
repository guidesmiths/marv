...
module.exports = function scanDirectory(directory, options, cb) {
  ...
  function ensureFilesNotFolders(files, cb) {
    debug('Ensuring no files read from directory %s are folders', directory);
    var filePaths = _.map(files, function(file) {
      return path.join(directory, file);
    });
    async.mapSeries(filePaths, fs.lstat, function(err, results) {
      if (err) return cb(err);
      var directoryItemsWithoutFolders = _.filter(files, function(file, index) {
