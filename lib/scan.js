...
module.exports = function scanDirectory(...args) {
  ...
  function getMigrations(files, cb) {
    async.reduce(files, [], (migrations, file, cb) => {
  ...
  function buildMigration(file, script, cb) {
    ...
    const level = parseInt(match.level, 10);
    const comment = match.comment.replace(/[-_]+/g, ' ');
